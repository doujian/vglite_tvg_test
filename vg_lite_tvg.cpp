 ...
 * @brief VGLite ThorVG backend implementation
 * 
 * Independent implementation without LVGL dependencies.
 */
...
// Logging macros (independent of LVGL)
#define VGLITE_LOG_ERROR(fmt, ...) fprintf(stderr, "[VGLITE ERROR] " fmt "\n", ##__VA_ARGS__)

#define TVG_CHECK_RETURN_VG_ERROR(FUNC)                               \
    do {                                                              \
        Result res = FUNC;                                            \
        if (res != Result::Success) {                                 \
            VGLITE_LOG_ERROR("Executed '" #FUNC "' error: %d", (int)res); \
...
static Result shape_append_path(Shape * shape, vg_lite_path_t * path, vg_lite_matrix_t * matrix);
...
static Result picture_load(vg_lite_ctx * ctx, Picture * picture, const vg_lite_buffer_t * source,
...
extern "C" {
    ...
    vg_lite_error_t vg_lite_clear(vg_lite_buffer_t * target, vg_lite_rectangle_t * rectangle, vg_lite_color_t color)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(shape->blend(BlendMethod::Normal));
        TVG_CHECK_RETURN_VG_ERROR(shape->fill(TVG_COLOR(color)));
        TVG_CHECK_RETURN_VG_ERROR(ctx->canvas->add(shape));
                                 ...
                                 vg_lite_filter_t filter)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(ctx->canvas->add(picture));
                                      ...
                                      vg_lite_filter_t filter)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(picture->blend(blend_method_conv(blend)));
        TVG_CHECK_RETURN_VG_ERROR(picture->mask(shape, MaskMethod::Alpha));
        TVG_CHECK_RETURN_VG_ERROR(ctx->canvas->add(picture));
                                 ...
                                 vg_lite_color_t color)
    {
        ...
        if(blend == VG_LITE_BLEND_DST_IN) {
            ...
            TVG_CHECK_RETURN_VG_ERROR(mask->fillRule(fill_rule_conv(fill_rule)));
            ...
            TVG_CHECK_RETURN_VG_ERROR(picture->mask(mask, MaskMethod::InvAlpha));
            TVG_CHECK_RETURN_VG_ERROR(ctx->canvas->add(picture));
        ...
        TVG_CHECK_RETURN_VG_ERROR(shape->fillRule(fill_rule_conv(fill_rule)););
...
Empty_sequence_handler:
                                             ...
                                             vg_lite_filter_t filter)
    {
        ...
        TVG_CHECK_RETURN_VG_ERROR(radialGrad->radial(grad->radial_grad.cx, grad->radial_grad.cy, grad->radial_grad.r, grad->radial_grad.cx, grad->radial_grad.cy, 0.0f));
...
static Result shape_set_stroke(Shape * shape, const vg_lite_path_t * path)
{
                           ...
                           vg_lite_color_t color)
{
    ...
    TVG_CHECK_RETURN_RESULT(picture->load((uint32_t *)image_buffer, source->width, source->height, ColorSpace::ARGB8888, true));
